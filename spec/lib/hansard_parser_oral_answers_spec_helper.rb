...
module OralAnswersHelperMethods
  ...
  def parse_oral_answers_all
    if PARSED[@name]
      ...
      mp = double(Mp)
      mp.should_receive(:id).at_least(:once).and_return 42
...
shared_examples "All oral questions" do
...
module OralQuestionHelperMethods
  ...
  def parse_debate
    if PARSED[@name]
      ...
      answer_about = double(@about_type)
      answer_from = double(@answer_from_type)
      ...
      @asking_mp.should_receive(:id).at_least(:once).and_return @asking_mp_id
      Mp.should_receive(:from_name).with(@asking_mp_name, @date).and_return @asking_mp

      @answering_mp = double(Mp)
      @answering_mp.stub(:party).and_return nil
      @answering_mp.should_receive(:id).at_least(:once).and_return @answering_mp_id
      Mp.should_receive(:from_name).with(@answering_mp_name, @date).and_return @answering_mp
      Mp.should_receive(:from_name).with(@supplimentary_answerer_name, @date).at_least(:once).and_return @answering_mp

      @supplimentary_questioners = []
      @supplimentary_questioners_names.each_with_index do |name, index|
        @supplimentary_questioners << double(Mp)
        @supplimentary_questioners.last.stub(:id).and_return @supplimentary_questioners_ids[index]
        Mp.stub(:from_name).with(name, @date).and_return @supplimentary_questioners.last
      end

      @interjecters = []
      @interjecter_names.each_with_index do |name,index|
        @interjecters << double(Mp)
        @interjecters.last.stub(:party).and_return nil
        @interjecters.last.stub(:id).and_return @interjecter_ids[index]
        Mp.stub(:from_name).with(name, @date).and_return @interjecters.last
      end
